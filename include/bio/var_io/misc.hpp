...
//!\brief Whether the descriptor is a run-length-encoding special value.
constexpr bool is_rle_desc(bcf_type_descriptor const desc)
{
    return ((static_cast<uint8_t>(desc) & 8u) == 8u);
}

//!\brief Convert descriptor value to run-length-encoding special value.
...
//!\brief Convert descriptor value from run-length-encoding special value.
...
//!\brief Whether the value is any (non-rle) integer value.
